...
import {upsertTestData} from '../redux/actions/templateAction';
...
class CreateTestSet extends Component {
  ...
  closeModal = () => {
    this.setState(
        {modalIsOpen: false, newTestSet: JSON.stringify(this.props.emptyTestSet, null, '\t'), newTestSetName: ''});
  };
  ...
  render() {
    return (
        <div style={this.props.style}>
          {this.props.templateIsSelected &&
          <Knapp type="standard" style={{marginTop: '30px'}} onClick={() => this.openModal()}>Legg til nytt
            testsett</Knapp>}
          <Modal
...
const mapDispatchToProps = dispatch => ({
  saveNewTestSet: (templateName, content, name) => dispatch(upsertTestData(templateName, content, name)),
});
